package com.laby.framework.websocket.core.sender.rabbitmq;
...
@RabbitListener(
        bindings = @QueueBinding(
                value = @Queue(
                        ...
                        name = "${laby.websocket.sender-rabbitmq.queue}" + "-" + "#{T(java.util.UUID).randomUUID()}",
                        // Consumer 关闭时，该队列就可以被自动删除了
                        autoDelete = "true"
                ),
                exchange = @Exchange(
                        name = "${laby.websocket.sender-rabbitmq.exchange}",
